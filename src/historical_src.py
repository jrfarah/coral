...
def get_percentages(im):
	...
	with open("C:\Users\jrfar\Documents\python\coral\db\hist.db", "a") as database:
...
def analyze_images():
	for root, dirs, files in os.walk('C:\Users\jrfar\Documents\python\coral\historical_frames'):
		for file in files:
			file_path = os.path.normpath('C:/Users/jrfar/Documents/python/coral/historical_frames/'+file)
...
menubar.add_command(label="Save the current graph!", command=lambda:generate_graphs("C:\Users\jrfar\Documents\python\coral\db\hist.db"))

Button(main,text='Get percentages!', command=lambda:analyze_images()).grid(row = 0, column=1)
Button(main,text='Show graph', command=lambda:generate_graphs("C:\Users\jrfar\Documents\python\coral\db\hist.db")).grid(row = 0, column=2)
